...
TEST(DFCXXVarsConstant, AddSameConstant) {
  ...
  DFVariableImpl *const1 = DFConstant::createOrUseConst(type, val);

  DFVariableImpl *const2 = DFConstant::createOrUseConst(type, val);
...
TEST(DFCXXVarsConstant, AddSameConstantDifferentWidth) {
  ...
  DFVariableImpl *const1 = DFConstant::createOrUseConst(type1, val);

  DFTypeImpl *type2 =
      meta.typeBuilder.buildFixed(FixedType::SignMode::UNSIGNED, 20, 0);
  meta.storage.addType(type2);
  DFVariableImpl *const2 = DFConstant::createOrUseConst(type2, val);
