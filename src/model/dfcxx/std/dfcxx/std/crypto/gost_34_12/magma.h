...
class MagmaBase : public dfcxx::Kernel {
public:
  ::std::string_view getName() const override {
    return "MagmaBase";
  }

  ~MagmaBase() override = default;
...
class MagmaEncoder : public MagmaBase {
public:
  ::std::string_view getName() const override {
    return "MagmaEncoder";
  }

  MagmaEncoder() : MagmaBase() {
...
class MagmaDecoder : public MagmaBase {
public:
  ::std::string_view getName() const override {
    return "MagmaDecoder";
  }

  MagmaDecoder() : MagmaBase() {
    const DFType ioType = dfUInt(64);
    DFVariable encoded = io.input("encoded", ioType);
    DFVariable key = io.input("key", dfUInt(256));
    auto curr = ::std::make_pair(encoded(63, 32), encoded(31, 0));
    ...
    block.connect(curr.second.cat(curr.first));
  }
};
