...
class TVM_DLL DeviceAPI {
 public:
   ...
   */
  virtual void SetDevice(DLDevice dev) = 0;
   ...
   */
  virtual void* AllocDataSpace(DLDevice dev, size_t nbytes, size_t alignment,
   ...
   * \return The allocated device pointer.
   */
  virtual void* AllocDataSpace(DLDevice dev, int ndim, const int64_t* shape, DLDataType dtype,
   ...
   */
  virtual void FreeDataSpace(DLDevice dev, void* ptr) = 0;
  /*!
   ...
   */
  virtual TVMStreamHandle CreateStream(DLDevice dev);
   ...
   */
  virtual void FreeStream(DLDevice dev, TVMStreamHandle stream);
   ...
   */
  virtual void StreamSync(DLDevice dev, TVMStreamHandle stream) = 0;
  /*!
   * \brief Set the stream
   * \param dev The device to set stream.
   * \param stream The stream to be set.
   */
  virtual void SetStream(DLDevice dev, TVMStreamHandle stream) {}
  /*!
   ...
   */
  virtual void SyncStreamFromTo(DLDevice dev, TVMStreamHandle event_src, TVMStreamHandle event_dst);
  /*!
   ...
   */
  virtual void* AllocWorkspace(DLDevice dev, size_t nbytes, DLDataType type_hint = {});
  /*!
   ...
   */
  virtual void FreeWorkspace(DLDevice dev, void* ptr);
   ...
   */
  static DeviceAPI* Get(DLDevice dev, bool allow_missing = false);
 ...
 */
inline bool IsRPCSessionDevice(DLDevice dev) { return (dev.device_type / kRPCSessMask) > 0; }
 ...
 */
inline DLDevice RemoveRPCSessionMask(DLDevice dev) {
 ...
 */
inline DLDevice AddRPCSessionMask(DLDevice dev, int session_table_index) {
