...
void Ge::estimate(const Parameters &params, Indicators &indicators) const {
...
std::shared_ptr<MetaElement> Ge::create(const NodeType &nodetype,
  ...
  return std::make_shared<Ge>(lowerCaseName, "std", false, params, ports);
}

std::unique_ptr<Element> Ge::construct() const {
...
bool Ge::isGe(const NodeType &nodetype) {
  return nodetype.inputs.size() == 2
    && nodetype.outputs.size() == 1
    && utils::starts_with(nodetype.name, "GE");
}
