...
void Shr14::estimate(const Parameters &params, Indicators &indicators) const {
...
std::shared_ptr<MetaElement> Shr14::create(const NodeType &nodetype,
  ...
  return std::make_shared<Shr14>(lowerCaseName, "std", true, params, ports);
}

std::unique_ptr<Element> Shr14::construct() const {
...
bool Shr14::isShr14(const NodeType &nodeType) {
  return nodeType.outputs.size() == 1 
      && nodeType.inputs.size()  == 1
      && utils::starts_with(nodeType.name, "SHR14");
}
