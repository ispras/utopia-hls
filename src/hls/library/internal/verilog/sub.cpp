...
void Sub::estimate(const Parameters &params, Indicators &indicators) const {
...
std::shared_ptr<MetaElement> Sub::create(const NodeType &nodetype,
  ...
  return std::make_shared<Sub>(lowerCaseName, "std", false, params, ports);
}

std::unique_ptr<Element> Sub::construct() const {
