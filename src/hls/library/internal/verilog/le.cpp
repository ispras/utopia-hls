...
void Le::estimate(const Parameters &params, Indicators &indicators) const {
...
std::shared_ptr<MetaElement> Le::create(const NodeType &nodetype,
  ...
  return std::make_shared<Le>(lowerCaseName, "std", false, params, ports);
}

std::unique_ptr<Element> Le::construct() const {
...
bool Le::isLe(const NodeType &nodetype) {
  return nodetype.inputs.size() == 2
    && nodetype.outputs.size() == 1
    && utils::starts_with(nodetype.name, "LE");
}
