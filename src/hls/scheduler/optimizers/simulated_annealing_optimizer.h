...
namespace eda::hls::scheduler::optimizers {
    ...
    class simulated_annealing_optimizer {
      ...
      private:
        float get_probabiliy(const float& prev_f, const float& cur_f, const float& cur_lim, const float& temp);
        ...
        std::function<float(const std::vector<float>&, float)> condition_function;
