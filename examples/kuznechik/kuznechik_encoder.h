...
public:
  ...
  DFVariable tablePermut(DFVariable val) {
  ...
  DFVariable S(DFVariable value) {
    DFVariable subst = tablePermut(value(127, 120));
    for (int i = 127; i >= 0; i -= 8) {
      subst = subst.cat(tablePermut(value(i, i - 7)));
    }
  ...
  DFVariable LGalois(DFVariable val) {
    ...
    DFVariable result = galois8Mul(consts[0], val(127, 120));

    for (uint8_t i = 1; i < 16; ++i) {
      uint8_t ind = 127 - 8 * i;
      result = result ^ galois8Mul(consts[i], val(ind, ind - 7));
    }

    return result;
  }

  DFVariable R(DFVariable val) {
    return LGalois(val).cat(val(127, 8));
  }

  DFVariable L(DFVariable val) {
