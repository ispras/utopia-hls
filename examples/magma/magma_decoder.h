...
class MagmaDecoder : public dfcxx::Kernel {
public:
  std::string_view getName() const override {
    return "MagmaDecoder";
  }

  ~MagmaDecoder() override = default;
  ...
  DFVariable magmaIter(DFVariable left, DFVariable right, DFVariable key) {
    ...
    DFVariable sum = left.cast(type) + key.cast(type);
  ...
  MagmaDecoder() : dfcxx::Kernel() {
    const DFType ioType = dfUInt(64);

    DFVariable encoded = io.input("encoded", ioType);
    DFVariable key = io.input("key", dfUInt(256));

    DFVariable currLeft = encoded(31, 0);
    DFVariable currRight = encoded(63, 32);

    for (int kInd = 0; kInd < 8; ++kInd) {
      int currKInd = 255 - 32*kInd;
      DFVariable buf = currLeft;
      currLeft = magmaIter(currLeft, currRight, key(currKInd, currKInd - 31));
      currRight = buf;
    }
    ...
    block.connect(currLeft.cat(currRight));
  }
};
