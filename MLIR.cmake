...
# Utopia uses exceptions, so we need to make sure that LLVM_REQUIRES_EH is set to ON, so that
...
# If, for some reason, it is not set, default to an empty string which is the old default behavior of Utopia.
set(UTOPIA_MLIR_DEFAULT_TRIPLE "${LLVM_HOST_TRIPLE}" CACHE STRING "Default triple for Utopia.")
message(STATUS "UTOPIA_MLIR_DEFAULT_TRIPLE: " ${UTOPIA_MLIR_DEFAULT_TRIPLE})
...
function(add_utopia_mlir_dialect_doc dialect dialect_tablegen_file)
  # Generate Dialect Documentation
  set(LLVM_TARGET_DEFINITIONS ${dialect_tablegen_file})
  tablegen(MLIR ${dialect}.md -gen-op-doc "-I${UTOPIA_MLIR_SRC_ROOT}")
  set(GEN_DOC_FILE ${UTOPIA_MLIR_BIN_ROOT}/docs/Dialects/${dialect}.md)
  add_custom_command(
          OUTPUT ${GEN_DOC_FILE}
  ...
  add_dependencies(utopia-mlir-doc ${dialect}DocGen)
endfunction()
add_custom_target(utopia-mlir-doc)

function(add_utopia_mlir_dialect dialect)
  set(LLVM_TARGET_DEFINITIONS ${dialect}.td)
  mlir_tablegen(${dialect}.hpp.inc -gen-op-decls "-I${UTOPIA_MLIR_SRC_ROOT}")
  mlir_tablegen(${dialect}.cpp.inc -gen-op-defs "-I${UTOPIA_MLIR_SRC_ROOT}")
  add_public_tablegen_target(OM${dialect}IncGen)
endfunction()

function(add_utopia_mlir_rewriter rewriter)
  set(LLVM_TARGET_DEFINITIONS ${rewriter}.td)
  mlir_tablegen(UTOPIA${rewriter}.inc -gen-rewriters "-I${UTOPIA_MLIR_SRC_ROOT}")
  add_public_tablegen_target(OMUTOPIA${rewriter}IncGen)
endfunction()

function(add_utopia_mlir_interface interface)
...
# add_utopia_mlir_library(name sources...
...
#   3. Add the library to a global property UTOPIA_MLIR_LIBS
#   4. Add an install target for the library
#   EXCLUDE_FROM_OM_LIBS
#     Do not add the library to the UTOPIA_MLIR_LIBS property.
#   NO_INSTALL
...
#   )
function(add_utopia_mlir_library name)
  ...
  if (NOT ARG_EXCLUDE_FROM_OM_LIBS)
    set_property(GLOBAL APPEND PROPERTY UTOPIA_MLIR_LIBS ${name})
  endif()
  ...
  target_include_directories(${name}
    PUBLIC
    ${UTOPIA_MLIR_SRC_ROOT}
    ${UTOPIA_MLIR_BIN_ROOT}
    )
...
endfunction(add_utopia_mlir_library)

# add_utopia_mlir_executable(name sources...
...
#     Same semantics as target_link_libraries().
#   )
function(add_utopia_mlir_executable name)
  cmake_parse_arguments(ARG
    "NO_INSTALL"
    ""
    "DEPENDS;INCLUDE_DIRS;LINK_LIBS"
...
endfunction(add_utopia_mlir_executable)
